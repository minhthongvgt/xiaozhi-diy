...
SpiLcdDisplay::SpiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    // draw white using DMA capable buffer
    uint16_t* line_buf = (uint16_t*)heap_caps_malloc(width_ * sizeof(uint16_t), MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
    if (line_buf != nullptr) {
        std::fill_n(line_buf, width_, 0xFFFF);
        for (int y = 0; y < height_; y++) {
            esp_lcd_panel_draw_bitmap(panel_, 0, y, width_, y + 1, line_buf);
        }
    ...
    ESP_LOGI(TAG, "Turning display on");
    {
        ...
        if (__err == ESP_ERR_NOT_SUPPORTED) {
            ...
            ESP_LOGW(TAG, "Panel disp_on_off returned %s", esp_err_to_name(__err));
        }
    }
...
void LcdDisplay::SetupUI() {
    ...
    if (display_ == nullptr) {
        ESP_LOGE(TAG, "Cannot SetupUI: display_ is null");
        return;
    }
    ...
    if (screen == nullptr) {
        ESP_LOGE(TAG, "Cannot SetupUI: active screen is null");
        return;
    }
